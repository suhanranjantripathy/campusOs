...
import { scrapeAssignments } from "@/lib/newton/scraper";
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log("[API] Scraping assignments data...");
    const data = await scrapeAssignments(session);

    return NextResponse.json({ success: true, data });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to fetch assignments";
    console.error("[API] Assignments scrape error:", message);
